    ...
    $.fn.thumbox = function(settings) {
        ...
        var attatchWheelEvent = function(){
            if(document.attachEvent) {
                ...
                if(window.addEventListener) window.addEventListener($.support.mozilla ? 'DOMMouseScroll' : 'mousewheel', thumboxWheel, false);
            }
        }
        ...
        var deattatchWheelEvent = function(){
            if(document.detachEvent) {
                ...
                if(window.removeEventListener) window.removeEventListener($.support.mozilla ? 'DOMMouseScroll' : 'onmousewheel', thumboxWheel, false);
            }
        }
        
        var thumboxWheel = function(event){
			if (($.support.msie && (Number($.support.version) < 8)) || $.support.opera) {
				return;
			}
            var delta = 0;
            if (!event) 
        ...
        var showImage = function(cord, urlImg, label, imgindex){
            ...
            objImagePreloader.onload  = function(){
                ...
                img.src   = urlImg;
                imgOriWidth  = objImagePreloader.width;
                imgWidth  = $(window).width()*0.7;
                if(imgOriWidth < imgWidth){
                	imgWidth = imgOriWidth;
                }
                imgOriHeight = objImagePreloader.height;
                imgHeight  = $(window).height()*0.7;
                
                if(imgOriHeight < imgHeight){
                	imgHeight = imgOriHeight;
                }
                var imgTop = ($(window).height()/2)-(imgHeight/2)+$(document).scrollTop();
                
                if (typeof tTop == 'undefined') {
                	tTop = 0;
                }
