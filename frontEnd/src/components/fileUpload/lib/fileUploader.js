...
class FileUploader{
  ...
  */
  uploadItem(fileItem){
    ...
    if(this.method === "RESOURCE" ) postMethod = "_resource";
  ...
  */
  _onSuccessUpload(fileItem, response, status, headers){
    var fileUrl = response.json()['data']['imgurl']
    if( typeof(fileUrl)=='string'){
      fileItem.onSuccess(fileUrl);
  ...
  */
  _resource(filecontrol){
    ...
    this.vm.$http.post(filecontrol.url, filecontrol.file, {
      params: uploader.formData,
      headers:{"Content-Type": filecontrol.type},
      // use before callback
      before(request) {
        ...
        uploader._onAbortUpload(filecontrol, null, 0, {});
        uploader._onCompleteUpload(filecontrol, null, 0, {});
      }
    }
    ).then((response) => {
      uploader._onSuccessUpload(filecontrol, response, response.status, response.headers);
      uploader._onCompleteUpload(filecontrol, response, response.status, response.headers);
    }, (response) => {
      uploader._onErrorUpload(filecontrol,response,response.status,response.headers);
      uploader._onCompleteUpload(filecontrol, response, response.status, response.headers);
    })
  }
}
